    ...
    for (const node of nodes) {
      if (node.type === 'text' && node.value.includes('\n')) {
        ...
        for (let i = 1; i < valueLines.length; i++) {
          ...
          if (valueLines[i]) {
            // Prepend non-breaking spaces to preserve indentation in text
            const textWithIndent = '\u00A0'.repeat(spaces) + valueLines[i];
            currentGroup.nodes.push({ ...node, value: textWithIndent });
          }
